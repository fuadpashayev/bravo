...
class MediaController extends Controller
{
        ...
        switch ($request->input('view_in')) {
            case 'all_media':
                $breadcrumbs = [
                    [
                        'id'   => 0,
                        'name' => translate('media.all_media'),
            ...
            case 'favorites':
                $breadcrumbs = [
                    [
                        'id'   => 0,
                        'name' => translate('media.favorites'),
                        'icon' => 'fa fa-star',
                    ],
                ];
    ...
    {
        $response = RvMedia::responseError(translate('media.invalid_action'));
        ...
        switch ($type) {
            case 'trash':
                ...
                if ($error) {
                    $response = RvMedia::responseError(translate('media.trash_error'));
                    break;
                }

                $response = RvMedia::responseSuccess([], translate('media.trash_success'));
                break;
            ...
            case 'restore':
                ...
                if ($error) {
                    $response = RvMedia::responseError(translate('media.restore_error'));
                    break;
                }

                $response = RvMedia::responseSuccess([], translate('media.restore_success'));
                break;
            ...
            case 'make_copy':
                ...
                $response = RvMedia::responseSuccess([], translate('media.copy_success'));
                break;
            ...
            case 'delete':
                ...
                $response = RvMedia::responseSuccess([], translate('media.delete_success'));
                break;
            case 'favorite':
                ...
                $response = RvMedia::responseSuccess([], translate('media.favorite_success'));
                break;
            ...
            case 'remove_favorite':
                ...
                $response = RvMedia::responseSuccess([], translate('media.remove_favorite_success'));
                break;
            ...
            case 'rename':
                ...
                if (!empty($error)) {
                    $response = RvMedia::responseError(translate('media.rename_error'));
                    break;
                }

                $response = RvMedia::responseSuccess([], translate('media.rename_success'));
                break;
            ...
            case 'empty_trash':
                ...
                $response = RvMedia::responseSuccess([], translate('media.empty_trash_success'));
                break;
        }
        ...
        if (count($items) == 1 && $items['0']['is_folder'] == 'false') {
            ...
            if (!empty($file) && $file->type != 'video') {
                if (config('filesystems.default') == 'local') {
                    $file_path = public_path($file->url);
                    if (!file_exists($file_path)) {
                        return RvMedia::responseError(translate('media.file_not_exists'));
                    }
            ...
            return RvMedia::responseError(translate('media.download_file_error'));
        }
        return RvMedia::responseError(translate('media.can_not_download_file'));
    }
}
